...
const db = new pg.Client({
  user: 'postgres',
  host: 'localhost',
  password: 'pw', //enter your postgres pw here
  database: 'world',
  port: 5432,
});
...
app.post("/change", async (req,res) => {
  ...
  try {
    ...
    if (submittedButton === 'submit') {
      //adding the entered country to the list for the user
      try {
        await db.query(
      ...
      //removing entered country from the list of this user
    } else {
...
//delete all countries from the current user's list
app.get("/delete", async (req, res) => {
  await db.query("DELETE FROM visited_countries WHERE user_id=$1", [currentUserId]);
  res.redirect('/');
});

//choosing user's tab or creating a new user
